...
Shader "Shader Forge/Gatu_shader8_Tech" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 lightColor = _LightColor0.rgb;
////// Lighting:
                float node_8091 = pow(max(0,dot(lightDirection,viewReflectDirection)),exp2(_Spec_gloss));
                float3 finalColor = (((node_8091*_Spec_Intensity)+tex2D( _GrabTexture, (_Dep*(_Hei - _Ref)*mul(tangentTransform, viewDirection).xy + (i.screenPos.rg*0.5+0.5)).rg).rgb)*_LightColor0.rgb);
