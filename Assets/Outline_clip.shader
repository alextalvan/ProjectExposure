...
Shader "Custom/Outline_clip" {
    Properties {
        ...
        _Emssion_Tint ("Emssion_Tint", Color) = (1,1,1,1)
...
////// Emissive:
                ...
                float3 emissive = (((1.0 - ceil(((1.0 - ((atan2(node_2130.g,node_2130.r)/6.28318530718)+0.5))-_Clip)))*(_Emssion_Texture_var.rgb*_Emssion_Tint.rgb*_Emission_Intensity))+(_Diffuse_Texture_var.rgb*_Diffuse_tint.rgb));
                float3 finalColor = emissive + (_LightColor0.rgb*attenuation*((_Light_color.rgb*max(0,dot(normalDirection,lightDirection)))*(pow(max(0,dot(lightDirection,viewReflectDirection)),exp2(_Spec_Power))*_Spec_Intensity)));
...
////// Lighting:
                float attenuation = LIGHT_ATTENUATION(i);
                float3 finalColor = (_LightColor0.rgb*attenuation*((_Light_color.rgb*max(0,dot(normalDirection,lightDirection)))*(pow(max(0,dot(lightDirection,viewReflectDirection)),exp2(_Spec_Power))*_Spec_Intensity)));
